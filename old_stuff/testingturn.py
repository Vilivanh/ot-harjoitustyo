...
if starter == "pelaaja 1":
    starting = "1"
elif starter == "pelaaja 0":
    starting = "0"
elif starter == "pelaaja 2":
    starting = "2"
elif starter == "pelaaja 3":
    starting = "3"
...
while running:
    ...
    if len(tabledeck) > 0:
        ...
        for i in range(len(tabledeck)):
            ...
            image = pygame.image.load(os.path.join(dirname, "assets", filename))
            image_scaled = pygame.transform.scale(image, DEFAULT_IMAGE_SIZE)
            card_position = pygame.Rect(250+i*60, 350, 59, 145)
            
            clickhandles.append((card_position, rank, suit))
            screen.blit(image_scaled, (250+i*60,350))
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            running = False   
    if starting == "0":
        players_number = 4
        PlayerTurn.players_turn(tabledeck, playerlists, players_number)
        
    elif starting == "1":
        ComputerTurn.computers_turn(tabledeck, playerlists, 4, 1)
        
    elif starting == "2":
        ComputerTurn.computers_turn(tabledeck, playerlists, 4, 2)
        
    elif starting == "3":
        ComputerTurn.computers_turn(tabledeck, playerlists, 4,3)    
    pygame.display.flip()
